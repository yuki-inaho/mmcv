...
HOST_DEVICE_INLINE int convex_hull_graham(const Point<T> (&p)[24],
  ...
  // Normalize the cross-product comparison by vector length so the Graham-scan
  // angular sort is not biased by point distance (open-mmlab/mmcv#2964 / #2933).
  for (int i = 0; i < num_in; i++) {
...
#else
  // CPU version
  // Normalize by vector length so the angular sort is not biased by distance
  // (open-mmlab/mmcv#2964 / #2933).
